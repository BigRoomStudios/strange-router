...
internals.renderRoutes = (routes) => {
    ...
    const absolutizedRoutes = routes.map(internals.rAbsolutizePath.bind(null, '/'));

    // Deconstruct the config so we end up with a flat array of routes
    const flattenedRoutes = internals.rFlattenArray(absolutizedRoutes.map(internals.rFlattenChildRoutes))
        .filter((rt) => typeof rt.component !== 'undefined'); // Remove structural routes that don't have components defined
...
internals.rAbsolutizePath = (pathPrefix, route) => {
    ...
    const boundAbsolutizePathFunc = internals.rAbsolutizePath.bind(null, path);
...
internals.buildRoutesFromAbsolutePaths = (absolutizedRoutes) => {
    ...
    const structuredRoot = internals.rGetChildRoutesForBase(rootChildren, rootRoute);

    return internals.rDedupeChildRoutes(structuredRoot.childRoutes);
};

internals.rGetChildRoutesForBase = (usingRoutes, baseRoute) => {
    ...
    clone.childRoutes = clone.childRoutes.map(internals.rGetChildRoutesForBase.bind(null, usingRoutes))
        .sort(internals.sortRoutes(baseRoute.path));

    return clone;
};

internals.rFlattenChildRoutes = (route) => {
...
internals.rFlattenArray = (arr) => {
    ...
    arr.forEach((arrItem) => {
        ...
        if (Array.isArray(arrItem)) {
            ...
            flat = flat.concat([...internals.rFlattenArray(arrItem)]);
        }
        else {
            flat.push(arrItem);
        }
...
internals.rDedupeChildRoutes = (routes) => {
    ...
    return routes.filter((r) => {
        ...
        return !allChildRoutes.find((cr) => cr.path === r.path);
    })
    .map((r) => {
