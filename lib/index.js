...
exports.buildRoutes = (routes) => {
    ...
    const toRender = internals.flatten(routes.map((r) => {
        ...
        return (
            <Route
                ...
                render={(props) => {
                    ...
                    const childSwitcher = route.childRoutes ? <Switch>{route.childRoutes.map(internals.renderRoute(normalizedPath))}</Switch> : null;
